...
describe("Households + Power Plants", function () {
    ...
    describe('complex network', () => {
        ...
        forEach([
            ...
            world.connectHouseholdToPowerPlant(households[getRandomInt(households.length)], powerPlant1);
            world.connectHouseholdToPowerPlant(households[getRandomInt(households.length)], powerPlant2);
            
            households.forEach((household, index) => {
                world.connectHouseholdToHousehold(household, households[index === households.length - 1 ? 0 : index + 1])
            });

            world.connectHouseholdToHousehold(households[getRandomInt(households.length)], households[getRandomInt(households.length)])
            world.connectHouseholdToHousehold(households[getRandomInt(households.length)], households[getRandomInt(households.length)])
            
            assert.equal(world.householdHasEletricity(households[getRandomInt(households.length)]), true);
            assert.equal(world.householdHasEletricity(households[getRandomInt(households.length)]), true);
            ...
            assert.equal(world.householdHasEletricity(households[getRandomInt(households.length)]), false);
            assert.equal(world.householdHasEletricity(households[getRandomInt(households.length)]), false);
        });
    });
});
